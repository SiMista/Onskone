...
// Contour renforcé pour les gros emojis des cartes-thèmes (44px) : 8 directions + ombre marquée.
const STRONG_STICKER_FILTER =
...
const ThemePickerModal = ({ isOpen, onClose, catalog, selected, mode, hostName, onChange }: ThemePickerModalProps) => {
  ...
  const handleToggle = (cat: string, code: string) => {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex flex-col bg-black/70 backdrop-blur-md animate-modal-backdrop"
      onClick={onClose}
      style={{
    ...
    >
      ...
      <div
        className="relative shrink-0 flex items-start justify-between gap-3 px-5 pb-1.5"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h2 className="text-lg md:text-xl font-display font-bold text-white m-0 tracking-tight drop-shadow-[0_2px_3px_rgba(0,0,0,0.6)]">
              {t.themePicker.title}
            </h2>
            <span className="inline-flex items-center text-xs font-display font-bold text-white/90 bg-black/30 border border-white/25 rounded-full px-2 py-0.5">
              {t.themePicker.counter(totals.sel, totals.total)}
            </span>
          </div>
          <p className="m-0 inline-flex items-center gap-1.5 text-xs font-sans italic text-white/75 leading-tight">
        ...
        <div className="relative h-full border-[2.5px] border-black rounded-2xl bg-white overflow-hidden stack-shadow-lg flex flex-col">
          ...
          {activeDescription && (
            ...
            >
              ...
              <span className="relative font-sans text-xs italic text-gray-700 leading-snug">
                {activeDescription}
              </span>
            </div>
          )}
            ...
            {activeThemes.map((info: ThemeInfo) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  disabled={!isEditable}
                  onClick={() => handleToggle(activeCategory, info.code)}
                ...
                >
                  ...
                  >
                    ...
                    <Icon
                      ...
                      style={{ filter: STRONG_STICKER_FILTER }}
                    />
                  </div>
                  {/* Texte */}
                  <div className="flex-1 min-w-0 px-3.5 py-3 md:px-4 md:py-3.5 flex flex-col justify-center gap-1">
                    <div className="font-display font-bold text-display-md text-gray-900 leading-tight line-clamp-1">
                      {info.name}
                    </div>
                    {info.description && (
