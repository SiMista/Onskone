    ...
    >
      ...
      <div className="sticky top-0 z-30 backdrop-blur-xl bg-[#0a0c12]/85 border-b border-white/[0.07]">
        <div className="px-4 py-3 flex flex-wrap items-center gap-2.5">
          ...
          {/* View switcher : un seul toggle, clic n'importe où = bascule */}
          <button
            type="button"
            role="switch"
            aria-checked={view === 'gallery'}
            aria-label="Basculer entre Régie et Composants"
            onClick={() => setView((v) => (v === 'rigging' ? 'gallery' : 'rigging'))}
          ...
          >
            {/* Thumb glissant (n'intercepte pas les clics) */}
            <span
              aria-hidden
          ...
          {view === 'rigging' && (
            <>
              ...
              {/* Cluster: toggles (icon-only pour rester sur une ligne) */}
              <div className={CLUSTER}>
                <button
                  type="button"
